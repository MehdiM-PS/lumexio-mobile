...
{
    return array_merge(sampleSupplierOrder(), [
        'items' => [
...
it('renders an inline top bar with the reference and a back button, hoisted into native chrome', function () {
    ...
    Native::test(SupplierOrderDetail::class, params: ['id' => 'xyz-789'], data: ['order' => sampleSupplierOrder(['id' => 'xyz-789'])])
...
it('paints instantly from navigation data then hydrates line items via loadDetail', function () {
    ...
    $screen = Native::test(SupplierOrderDetail::class, params: ['id' => 'xyz-789'], data: ['order' => sampleSupplierOrder(['id' => 'xyz-789'])]);
...
it('shows the receive button only when status is pending, and marks fully received via the real binding', function () {
    ...
    $screen = Native::test(SupplierOrderDetail::class, params: ['id' => 'xyz-789'], data: ['order' => sampleSupplierOrder(['id' => 'xyz-789', 'status' => 'commandee'])])
...
it('does not show the receive button when status is brouillon', function () {
    ...
    Native::test(SupplierOrderDetail::class, params: ['id' => 'xyz-789'], data: ['order' => sampleSupplierOrder(['id' => 'xyz-789', 'status' => 'brouillon'])])
